...
class OrientedSolver:
    ...
    def __init__(self, problem, pas = 0.001):
        """
        Parameters
        ----------
        problem : TSPProblem
    ...
    def plot(self):
        ...
        intervale_min = 0
        intervale_max = len(self.__solution_history) - 1
        ...
        def get_edge(index_solution):
            edge = []
            array = self.__solution_history[index_solution].T @ self.__A @ self.__solution_history[index_solution]
            for i in range(self.__problem.get_number()):
