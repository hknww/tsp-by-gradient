...
class OrientedSolver:
    ...
    def __init__(self, problem, pas = 0.00015):
        """
        Parameters
        ----------
        problem : TSPProblem
    ...
    def new_solution(self, solution_var, lambda_var):
        ...
        test_1 = p_dot
        test_2 = - lbd * P @ gnr_lie_bracket(indiv_product(P,P),P)
        p_dot = p_dot - lbd * P @ gnr_lie_bracket(indiv_product(P,P),P)
        # print(p_dot)
        self.grad_history += [np.linalg.norm(p_dot, 'fro')]
        self.test = [test_1,test_2]
    ...
    def lagrange(self, limit = 1e-8, n_max=10000):
        ...
        while self.breaker() > limit and count < n_max:
            count += 1
            ### Lagrange
            self.__save_new_solution(self.new_solution(self.get_current_solution(), 0))
